...
if($a>0){
	$p1=(($r1*$z1)+($r2*$z2)+($r3*$z3)+($r4*$z4)+($r5*$z5)+($r6*$z6)+($r7*$z7)+($r8*$z8))/$a;
	}
//	
...
$query = mysql_query("insert into kriteria values('','$username','$g1','$g2','$g3','$g4','$g5','$g6','$g7','$g8','$g9','$g10','$g11','$g12','$tanggal')") or die(mysql_error());

$query = mysql_query("insert into defuzifikasi values('','$username','$p1','$p2','$p3','$nilai','$jenis','$diagnosa','$pengobatan')") or die(mysql_error());

if ($query) {
	header('location:hd.php?message=success');
	}else{
	echo"gagal";
}
?>
